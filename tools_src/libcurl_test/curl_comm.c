...
static size_t down_write_func(void* ptr, size_t size, size_t nmemb, void* userdata)
{
	...
	if ((pctsi->cur_pos + (long)(size*nmemb)) <= pctsi->end_pos)
	{
		fseek(pctsi->fp, pctsi->cur_pos, SEEK_SET);
		written = fwrite(ptr, size, nmemb, pctsi->fp);
		pctsi->cur_pos += size*nmemb;
	}
	else
	{
		fseek(pctsi->fp, pctsi->cur_pos, SEEK_SET);
		written = fwrite(ptr, 1, pctsi->end_pos - pctsi->cur_pos + 1, pctsi->fp);
		pctsi->cur_pos = pctsi->end_pos;
	}
...
static int WorkThreadProc(void* param)
{
	...
	while (errct < MAX_ERR_RETRY_NUM)
	{
		...
		if (pctsi->flag)
		{
			sprintf(range, "%ld-%ld", pctsi->cur_pos, pctsi->end_pos);
		...
		if (res != CURLE_OK)
		{
			LOG_W("thrd[%d] curl_easy_perform() failed: %s", pctsi->id, curl_easy_strerror(res));
			errct++;
			curl_easy_cleanup(curl);
			continue;
		}
	...
	if (errct >= MAX_ERR_RETRY_NUM)
	{
		LOG_W("thrd[%d] download file failed: %s", pctsi->id, curl_easy_strerror(res));
		pctsi->ret = 1;
		return -1;
	}
...
int download_file(int thread_num, char* url, char* path, char* filename, int timeout, SHOW_DOWNLOAD_PROCESS_PROC proc)
{
	...
	else
	{
		...
		for (i = 0; i <= thread_num; i++)
		{
			...
			ctsi[ct].fp = fp;
			ctsi[ct].id = ct;
			ctsi[ct].cur_pos = ctsi[ct].start_pos;
			ctsi[ct].hMutex = hMutex;
			ctsi[ct].timeout = timeout;
			strcpy(ctsi[ct].url, url);
			hWorkHandle[ct] = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)WorkThreadProc, &ctsi[ct], 0, 0);
	...
	while (loop)
	{
		ret = WaitForMultipleObjects(ct, hWorkHandle, TRUE, 1000);
		switch (ret)
		{
		case WAIT_TIMEOUT:
			for (i = 0; i < ct; i++)
			{
				down_len += ctsi[i].cur_pos - ctsi[i].start_pos;
			}
			if (proc && flag == 1)
			{
				proc(down_len, file_len);
			}
			down_len = 0;
			break;
		case WAIT_OBJECT_0:
