...
int LH_Init(OUT LINK_HASH** ppHash, IN LINKHASH_HASH_FN_TYPE h, IN LINKHASH_COMP_FN_TYPE c)
{
	...
	*ppHash = (LINK_HASH*)malloc(sizeof(LINK_HASH));
	if (*ppHash == NULL)
	{
	...
	pHash->b = (LINK_HASH_NODE **)malloc(sizeof(LINK_HASH_NODE *)* LINK_HASH_MIN_NODES);
	if (pHash->b == NULL)
	{
		free(pHash);
...
static int LH_Expand(IN LINK_HASH* pHash)
{
	...
	*n2 = NULL;
	...
	if ((pHash->p) >= pHash->pmax)
	{
		...
		n = (LINK_HASH_NODE **)realloc(pHash->b, sizeof(LINK_HASH_NODE*) * j);
		if (n == NULL)
		{
...
static int LH_Contract(IN LINK_HASH* pHash)
{
	...
	if (pHash->p == 0)
	{
		n = (LINK_HASH_NODE **)realloc(pHash->b, sizeof(LINK_HASH_NODE*) * pHash->pmax);
		if (n == NULL)
		{
...
int LH_Insert(IN LINK_HASH* pHash, IN void* data, OUT void** old_data)
{
	...
	{
		//在链表尾部追加数据
		nn = (LINK_HASH_NODE *)malloc(sizeof(LINK_HASH_NODE));
		if (nn == NULL)
		{
